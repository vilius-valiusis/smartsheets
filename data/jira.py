...
smart_sheet_id_for_coverage_sheet = 6185156768753540
...
URL_PARAMS_JIRA = '&fields=customfield_12825,priority,issuetype,status,labels,' \
                  'key,summary,customfield_10007&maxResults=100&startAt='
...
def get_smartsheets_column_mappings():
    mapping = {}
    url = URL_SMARTSHEETS + str(smartSheetId)
    r = requests.get(url, headers=HEADER_SMARTSHEETS)
    if r.status_code != 200:
        print('Could not get the smartsheet')
    else:
        print(
            "The call to Smartsheets to get the smartsheets with the ID of {id} was successful".format(id=smartSheetId))
        for column in r.json()['columns']:
            column_id = column['id']
            column_title = column['title']
            mapping[column_title] = column_id
    pprint(mapping)
    COLUMN_MAPPINGS.s
    exit()
    pass
...
def build_story_reference_dict(jira_ticket_dict):
    ...
    for key, ticket in jira_ticket_dict.items():
        if ticket['type'] == 'Story' and ticket['isTestable']:
            if ticket['hasTestCase'] and not (ticket['status'] == 'Blocked' or ticket['status'] == 'Draft'):
                story_reference_dict['hasReference'].append(key)

            elif not ticket['hasTestCase'] and ticket['hasAssignedSprint']:
                story_reference_dict['hasNoReference'].append(key)

            elif (ticket['status'] == 'Blocked' or ticket['status'] == 'Draft') and ticket['hasTestCase']:
...
def set_column_data(json, desired_column_name, column_name, data, fm, rows=0, dm=None):
    if desired_column_name == column_name and rows == 0 and dm is not None:
        json.setdefault("cells", []) \
            .append({"columnId": COLUMN_MAPPINGS[column_name], "value": dm[column_name], "format": fm})
    if rows != 0:
        json.setdefault("cells", []) \
            .append({"columnId": COLUMN_MAPPINGS[column_name], "value": dm[column_name], "format": fm})
    return json
...
def build_data_mapping_dict(jira_ticket_dict, severity_dict, priority_dict, status_dict, type_dict,
                            story_reference_dict, list_of_references, total_jira_issues, open_bug_list, unlabeled_list):
    mapping_dict = {
        'References': len(list_of_references),
        'Cases w References': len(story_reference_dict['hasReference']),
        'Cases w/o References': len(story_reference_dict['hasNoReference']),
        'Refs in Draft/Blocked': len(story_reference_dict['hasReferenceButDraftOrBlocked']),
        'Total JIRA': total_jira_issues,
        'Total Fueled': len(jira_ticket_dict),
        'Bugs': len(type_dict['Bug']['tickets']),
        'Stories': len(type_dict['Story']['tickets']),
        'Sub-Task': len(type_dict['Sub-task']['tickets']),
        'Task': len(type_dict['Task']['tickets']),
        'Epic': len(type_dict['Epic']['tickets']),
        'Blocked': len(status_dict['Blocked']['tickets']),
        'Code Review': len(status_dict['Code Review']['tickets']),
        'Defects Found': len(status_dict['Defects Found']['tickets']),
        'Done': len(status_dict['Done']['tickets']),
        'Draft': len(status_dict['Draft']['tickets']),
        'In Progress': len(status_dict['In Progress']['tickets']),
        'QA Complete': len(status_dict['QA Complete']['tickets']),
        'QA in Progress': len(status_dict['QA in Progress']['tickets']),
        'Ready for DEV': len(status_dict['Ready for DEV']['tickets']),
        'Ready for QA': len(status_dict['Ready for QA']['tickets']),
        'Total Bugs': len(open_bug_list),
        'Open: Low': len(severity_dict['low']['tickets']),
        'Open: Medium': len(severity_dict['medium']['tickets']),
        'Open: High': len(severity_dict['high']['tickets']),
        'Open: Critical': len(severity_dict['critical']['tickets']),
        'Open: No Severity': len(severity_dict['none']['tickets']),
        'Open: P1': len(priority_dict['P1']['tickets']),
        'Open: P2': len(priority_dict['P2']['tickets']),
        'Open: P3': len(priority_dict['P3']['tickets']),
        'Open: P4': len(priority_dict['P4']['tickets']),
        'Open: P5': len(priority_dict['P5']['tickets']),
        'List of Stories that Need References': story_reference_dict['hasNoReference'],
        'List of Stories that Have References': story_reference_dict['hasReference'],
        'List of Refs for Stories in Draft/Blocked': story_reference_dict['hasReferenceButDraftOrBlocked'],
        'List of Blocked': status_dict['Blocked']['tickets'],
        'List of Code Review': status_dict['Code Review']['tickets'],
        'List of Defects Found': status_dict['Defects Found']['tickets'],
        'List of Done': status_dict['Done']['tickets'],
        'List of Draft': status_dict['Draft']['tickets'],
        'List of In Progress': status_dict['In Progress']['tickets'],
        'List of QA Complete': status_dict['QA Complete']['tickets'],
        'List of Ready for DEV': status_dict['Ready for DEV']['tickets'],
        'List of Ready for QA': status_dict['Ready for QA']['tickets'],
        'List of ALT tickets with no label': unlabeled_list
...
def create_rows(jira_ticket_dict, severity_dict, priority_dict, status_dict, type_dict, story_reference_dict,
                list_of_references, total_jira_issues, open_bug_list, unlabeled_list, dm):
    ...
    for i in range(0, len(jira_ticket_dict)):
        ...
        for k, v in COLUMN_MAPPINGS.items():
            if i == 0:
                set_column_data(json, k, 'References', dm=dm, fm=sf)
                set_column_data(json, k, 'Cases w References', dm=dm, fm=sf)
                set_column_data(json, k, 'Cases w/o References', dm=dm, fm=sf)
                set_column_data(json, k, 'Refs in Draft/Blocked', dm=dm, fm=sf)
                set_column_data(json, k, 'Total JIRA', dm=dm, fm=sf)
                set_column_data(json, k, 'Total Fueled', dm=dm, fm=sf)
                set_column_data(json, k, 'Bugs', dm=dm, fm=sf)
                set_column_data(json, k, 'Stories', dm=dm, fm=sf)
                set_column_data(json, k, 'Sub-Task', dm=dm, fm=sf)
                set_column_data(json, k, 'Task', dm=dm, fm=sf)
                set_column_data(json, k, 'Epic', dm=dm, fm=sf)
                set_column_data(json, k, 'Blocked', dm=dm, fm=sf)
                set_column_data(json, k, 'Code Review', dm=dm, fm=sf)
                set_column_data(json, k, 'Defects Found', dm=dm, fm=sf)
                set_column_data(json, k, 'Done', dm=dm, fm=sf)
                set_column_data(json, k, 'Draft', dm=dm, fm=sf)
                set_column_data(json, k, 'In Progress', dm=dm, fm=sf)
                set_column_data(json, k, 'QA Complete', dm=dm, fm=sf)
                set_column_data(json, k, 'QA in Progress', dm=dm, fm=sf)
                set_column_data(json, k, 'Ready for DEV', dm=dm, fm=sf)
                set_column_data(json, k, 'Ready for QA', dm=dm, fm=sf)
                set_column_data(json, k, 'Total Bugs', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: Low', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: Medium', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: High', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: Critical', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: No Severity', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: P1', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: P2', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: P3', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: P4', dm=dm, fm=sf)
                set_column_data(json, k, 'Open: P5', dm=dm, fm=sf)

            if i < len(story_reference_dict['hasNoReference']):
                set_column_data(json, k, 'List of Stories that Need References',
                                story_reference_dict['hasNoReference'][i], nf)

            if i < len(story_reference_dict['hasReference']):
                set_column_data(json, k, 'List of Stories that Have References',
                                story_reference_dict['hasReference'][i], nf)

            if i < len(story_reference_dict['hasReferenceButDraftOrBlocked']):
                set_column_data(json, k, 'List of Refs for Stories in Draft/Blocked',
                                story_reference_dict['hasReferenceButDraftOrBlocked'][i], nf)

            if i < len(status_dict['Blocked']['tickets']):
                set_column_data(json, k, 'List of Blocked', status_dict['Blocked']['tickets'][i], nf)

            if i < len(status_dict['Code Review']['tickets']):
                set_column_data(json, k, 'List of Code Review', status_dict['Code Review']['tickets'][i], nf)

            if i < len(status_dict['Defects Found']['tickets']):
                set_column_data(json, k, 'List of Defects Found', status_dict['Defects Found']['tickets'][i], nf)

            if i < len(status_dict['Done']['tickets']):
                set_column_data(json, k, 'List of Done', status_dict['Done']['tickets'][i], nf)

            if i < len(status_dict['Draft']['tickets']):
                set_column_data(json, k, 'List of Draft', status_dict['Draft']['tickets'][i], nf)

            if i < len(status_dict['In Progress']['tickets']):
                set_column_data(json, k, 'List of In Progress', status_dict['In Progress']['tickets'][i], nf)

            if i < len(status_dict['QA Complete']['tickets']):
                set_column_data(json, k, 'List of QA Complete', status_dict['QA Complete']['tickets'][i], nf)

            if i < len(status_dict['Ready for DEV']['tickets']):
                set_column_data(json, k, 'List of Ready for DEV', status_dict['Ready for DEV']['tickets'][i], nf)

            if i < len(status_dict['Ready for QA']['tickets']):
                set_column_data(json, k, 'List of Ready for QA', status_dict['Ready for QA']['tickets'][i], nf)

            if i < len(unlabeled_list):
                set_column_data(json, k, 'List of ALT tickets with no label', unlabeled_list[0], nf)
...
def delete_rows():
    ...
    listOfRowsToDelete = []
    listOfRowIndexes = []
    ids = [smartSheetId, smart_sheet_id_for_coverage_sheet]
    for id in ids:
        url = 'https://api.smartsheet.com/2.0/sheets/' + str(id)
        headers = {'Cache-Control': 'no-cache', 'Authorization': 'Bearer ' + smartSheetPassword}
        ...
        for rowIndex in listOfRowIndexes:
            ...
            url = 'https://api.smartsheet.com/2.0/sheets/' + str(id) + '/rows?ids='
...
def create_coverage_sheet(story_reference_dict):
    # This creates a separate, brief sheet for a coverage pie chart
    url = 'https://api.smartsheet.com/2.0/sheets/' + str(smart_sheet_id_for_coverage_sheet) + '/rows'
    headers = {'Cache-Control': 'no-cache', 'Authorization': 'Bearer ' + smartSheetPassword}
    true = 'true'
    ...
    json.setdefault("cells", []).append(
        {"columnId": 986977739794308, "value": len(story_reference_dict['hasNoReference']), "format": summaryFormat})
    ...
    json.setdefault("cells", []).append(
        {"columnId": 986977739794308, "value": len(story_reference_dict['hasReference']), "format": summaryFormat})
    r = requests.post(url, headers=headers, json=json)
    if r.status_code == 200:
...
def main():
    get_smartsheets_column_mappings()
    ...
    data_mapping_dict = build_data_mapping_dict(severity_dict, priority_dict, status_dict, type_dict,
                                                story_reference_dict, list_of_references, total_jira_issues,
                                                open_bug_list, unlabeled_list)
    delete_rows()
    create_coverage_sheet(story_reference_dict)
